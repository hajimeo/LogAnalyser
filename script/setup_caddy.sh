...
_import "utils.sh"
_import "setup_work_env.sh"


### Global variables for this script ##########################################
_ROOT_DIR="/var/www/loganalyser"
_HOST="0.0.0.0"
_PORT="48080"
...
function f_validations() {
    ...
    _is_in_path "curl" "${_install}" || return $?
    _is_in_path "rg" "${_install}" || return $?
    _is_in_path "pip" "${_install}" || return $?
    _is_in_path "php-fpm" "${_install}" || return $?
